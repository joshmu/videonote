...
export const ProjectsModal = ({
  ...
  const handleMousePosition = (inside: boolean, project: ProjectInterface): void => {
      ...
      <ModalInnerContainer>
        {projects.map((project) => (
          <div
            ...
            className={`${
              currentProject._id === project._id ? "bg-opacity-25" : " bg-opacity-0 cursor-pointer"
            } relative bg-themeSelectOpacity transition-colors duration-200 ease-out`}
          >
            <Select
              cursor={currentProject._id === project._id ? "cursor-default" : "cursor-pointer"}
            >
              <div onClick={() => handleSelection(project._id)} className="flex-1">
                <span className="text-sm capitalize">{project.title} </span>
                <span className="ml-2 text-xs text-themeText2">{project.notes.length} notes</span>
              </div>
