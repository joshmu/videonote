...
export const Parallax = ({ rate = 0, className = "", children, ...props }: ParallaxProps) => {
  ...
  useLayoutEffect(() => {
    ...
    const onResize = () => {
      ...
      const halfWindowY = globalThis.window.innerHeight / 2 + ref.current.clientHeight / 2;
      setOffsetTop(ref.current.offsetTop - halfWindowY);
      setMinHeight(calculateMinHeight(ref.current.offsetHeight - halfWindowY, rate));
    };
  ...
  return (
    <div ref={ref} style={{ minHeight, margin: "auto", width: "100%" }} className={className}>
